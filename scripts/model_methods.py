...
def build_data(
    ...
    # Load data
    input_data = load_data(frac=frac, seed=seed, verbose=verbose)
...
def load_data(
    ...
    """Load in the data, currently stored as two separate files.
...
def encode_targets(
    ...
    data[f"{col_name}_target"] = preprocessing.LabelEncoder().fit_transform(
    ...
    subset_data = (
        data[["sequence", f"{col_name}_target"]]
        .copy()
...
@typechecked
def stratify_by(
    X: ndarray, y: ndarray, stratify: bool = True, seed: int = SEED
) -> ExperimentDataSplit:
    ...
    Args:
        ...
        stratify (bool, optional): If True, data is split in a stratified fashion, using the class labels. Defaults to True.
        seed (int, optional): Random state for reproducibiltiy. Defaults to SEED.
...
def tokenize_data(
    ...
    # For printing output from parallel processing job
    if verbose:
        verbose_num = 1
    else:
        verbose_num = 0
    ...
    for strat_type in EXPERIMENT_KEYS[1]:
        if experiment_dict.get(f"stratify_{strat_type}"):
            ...
            for split in EXPERIMENT_KEYS[2]:
                ...
                input_ids = []
                attention_masks = []
                # Parallel pool to process sequence encoding in parallel
